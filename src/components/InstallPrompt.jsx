...
export const InstallPrompt = ({ onDismiss }) => {
  ...
  const { promptInstall, showInstructionsModal, closeInstructionsModal } = usePWAInstall();
  ...
  const handleInstallClick = async () => {
    ...
    } else if (result === 'declined' || result === 'unavailable' || result === 'error') {
      // ✅ Instructions modal will auto-show from usePWAInstall hook
      // Just update status for visual feedback
      setStatus('idle');
    } else {
      setStatus('idle');
    }
  ...
  const handleInstructionsClose = () => {
    ...
    onDismiss(); // Also dismiss the banner when instructions modal is closed
  };
  ...
  return (
    <>
      ...
      {/* ✅ Install Instructions Modal - Shows when native install fails */}
